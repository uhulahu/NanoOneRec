...
def k_core_filter_amazon23(reviews, asin2title, K=5, start_timestamp=None, end_timestamp=None):
    ...
    反复剔除"出现次数少于 K 的用户和商品"，直到整个数据集稳定下来（每个人都 ≥ K 次、每个商品都 ≥ K 次），因为每删掉一些user/item会影响其他item/user的交互次数。
...
def build_interaction_list_amazon23(reviews, user2index, item2index, asin2title):
    ...
    for u in tqdm(interact.keys(), desc="Building sequence windows"):
        ...
        for i in range(1, len(items)):
            ...
            interaction_list.append([
                u,                       # original user_id
                items[st:i],             # history asins  # 从起始到 i-1
                items[i],                # target asin  # 待预测 target
    ...
    interaction_list.sort(key=lambda x: int(x[-1])) # 按 target 交互时间戳排序
...
def parse_args():
    ...
    parser.add_argument("--output_path", type=str, default="./data/Amazon23",
                        help="Output folder")
...
def main():
    ...
    # Load metadata and reviews  (metadata用于item特征，reviews用于交互序列)
    asin2meta, asin2title = load_metadata_amazon23(args.metadata_file)
    reviews = load_reviews_amazon23(args.reviews_file)  # reviews 两个作用：提取用户交互历史，偏好对齐
    if len(reviews) == 0:
    ...
    # K-core filtering K核分解，常用的数据清洗方法，去除冷启动用户和长尾物品（交互次数 < 5 的item、user）
    ...
    # Build interaction_list for sequences 滑动窗口构造 hist-target 样本，[st:i]
    interaction_list = build_interaction_list_amazon23(
        reviews_filtered, user2index, item2index, asin2title
    )

    # Write train/valid/test atomic files 按target交互时间，划分并保存train/val/test数据集，train→valid→test 时间严格递增
    train_data, valid_data, test_data = write_atomic_files(
        args, interaction_list, user2index
    )

    # Build item features 从 metadata 构造商品特征 dict
    item2feature = build_item_features_amazon23(asin2meta, item2index)

    # Build review features 从 reviews 构造 review dict
