...
> ⚠️ **历史流水声明（2026-09-18 补）**：本文档记录的是 **2026-08-30 ~ 09-03** 的监控与修复过程，
> 是**当时**的真实状态，**正文不随项目演进更新**。阅读时请注意四点：
>
> 1. **路径**：文中引用的 `outputs/` 等目录**已删除**（磁盘清理，见 `PROGRESS.md`）；行号引用已随
>    代码演进失效，定位请以符号名为准（`grep -n '<符号>' <文件>`）。
> 2. **环境已换代**：文中是 **torch 2.3.0+cu121 / 4×RTX 4090 24GB**；现为
>    **torch 2.12.1+cu130 / 4×RTX 5090 32GB + ZeRO-2**（见 `docs/environment.txt`）。
> 3. **「后续修复」多数已完成**：例如「仅 rank0 生成时间戳」→ 已实现为 `make_run_dir_name()`
>    （`ds_zero2_patches.py`，`rl.py` 调用）。
> 4. **仍有效的部分**：`sft.py` 的两个 monkeypatch（带 `[MONITORING]` 标记）**至今仍在**；
>    firstdiff Bug 1 的修复形态与当前 `rl.py` 逐字吻合；pad==eos 死行误杀事故是 RL 阶段独有的
>    记录，`PROGRESS.md` 未收录 —— 这三处是本文档保留的价值所在。
