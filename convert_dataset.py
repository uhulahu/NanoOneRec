...
def load_dataset(data_dir: str, dataset_name: str, index_dir: str = None) -> Dict[str, Any]:
    # 2026-09-03: 目录重组后 item.json/.inter 在 raw/、index.json 在 sid/<变体>/，index_dir 可单独指定（默认同 data_dir）
    ...
    with open(os.path.join(index_dir, f'{dataset_name}.index.json'), 'r') as f:
    ...
    for split_name, split_data in splits.items():
        ...
        for line in split_data:
            ...
            row = {
                ...
                'history_item_sid': history_semantic_ids,  # Semantic IDs (with brackets)  格式：['<><><>', ...]
        ...
        if rows:
            ...
            output_file = os.path.join(output_dir, f'{category}_5_2018-10-2023-9.csv')
...
def main():
    ...
    parser.add_argument('--index_dir', type=str, default=None,
                       help='SID index.json 所在目录（目录重组后与 raw 分离，默认同 data_dir）')
    ...
    dataset_data = load_dataset(args.data_dir, args.dataset_name, args.index_dir)
    ...
    info_file = os.path.join(args.output_dir, 'info', f'{args.category}_5_2018-10-2023-9.txt')
