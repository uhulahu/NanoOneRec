...
def balanced_kmeans_level_constrained(X, K, max_iter=100, tol=1e-7, random_state=None, verbose=False):
    ...
    kmeans = KMeansConstrained(
        n_clusters=K,
        size_min=min_size,  # 最小簇大小
        size_max=max_size,  # 最大簇大小
        max_iter=max_iter,
        tol=tol,            # 收敛tolerance，基于欧氏距离
        random_state=random_state,
        n_init=3,           # 以不同的初始簇心运行的次数，并返回其中最优的结果（inertia 最小的）
        verbose=verbose,
        n_jobs=1            # 必须为1：n_jobs>1 时 joblib 会把大数组以只读 memmap 传给 worker，
                            # 而包内 Cython 的 _centers_dense 需要可写 buffer，报 "buffer source array is read-only"
    )
...
def residual_kmeans_constrained(X, K, L, max_iter=300, tol=1e-4, random_state=None, verbose=False):
    ...
    for l in range(L):  # 逐层训练 kmeans-constrained
...
def parse_args():
    ...
    parser.add_argument('--emb_path', type=str, default=None,
                        help="embedding npy 完整路径（2026-09-03 起必填：不再由 dataset/plm_name/suffix 拼接文件名）")
    parser.add_argument('--sid_out_dir', type=str, default=None,
                        help="codes/codebooks/index.json 输出目录（默认同 root）")
...
if __name__ == "__main__":
    ...
    sid_out_dir = args.sid_out_dir or args.root
    data_path = args.emb_path
    if data_path is None:
        print("Error: --emb_path 必填（目录重组后 embedding 文件名不再自动拼接）")
        exit(1)
    ...
    codes_plus_one = codes_all.T + 1  # +1应该是为了把0留给起始token？
    codes_df = pl.DataFrame({'codes': [list(c) for c in codes_plus_one]})

    # 添加 Extra Token 解决碰撞，但是没有碰撞的不添加，所以长度不是对齐的
    t4 = time.time()
    ...
    t5 = time.time()
    codes_json = {}
    for id, row in enumerate(codes_dedup.iter_rows(named=True)):  # 这里 id 就是行索引
        codes_ = []
        for i, code in enumerate(row['codes']):
            codes_.append(f'<{chr(97+i)}_{code}>')  # ['<a_5>', '<b_23>', '<c_55>']
        codes_json[str(id)] = codes_

    # Save JSON index 结构：{id1:['<a_5>', '<b_23>', '<c_55>'], id2:['...','...','...'], ...}
